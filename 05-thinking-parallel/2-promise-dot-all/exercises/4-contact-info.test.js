...
const contactDetails = async (ids = []) => {
  const results = await Promise.allSettled(ids.map(fetchUserById));

  return results.map((result, i) => {
    if (result.status === 'fulfilled') {
      const { id, email, phone, website } = result.value;
      return `${id}: ${email}, ${phone}, ${website}`;
    }
    return `404: Not Found: there is no user with id ${ids[i]}`;
  });
};
